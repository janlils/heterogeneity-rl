...
from codes.rl_common import action_mask_from_obs, action_masks_from_obs_batch, append_agent_id_feature
...
class MAPPOActorCritic(nn.Module):
    """
    MAPPO z decentralized actor i centralized critic.

    Actor korzysta z lokalnej obserwacji agenta.
    Critic korzysta z lokalnej obserwacji + globalnych agregatów rynku.
    """
    ...
    def __init__(
        ...
        self.actor_trunk = nn.Sequential(
            nn.Linear(actor_obs_dim, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
        )
        self.critic_trunk = nn.Sequential(
            nn.Linear(critic_obs_dim, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
        )
    ...
    def critic_forward(self, critic_obs: torch.Tensor) -> torch.Tensor:
        h = self.critic_trunk(critic_obs)
...
class IndependentHeadPPOActorCritic(nn.Module):
    """
    Light-IPPO:
      - wspólny trunk dla wszystkich agentów
      - osobna głowa actor i critic dla każdego agenta
    """
    ...
    def __init__(self, obs_dim: int, n_actions: int, n_agents: int, hidden_size: int = 64):
        ...
        self.policy_heads = nn.ModuleList([
            nn.Linear(hidden_size, n_actions) for _ in range(n_agents)
        ])
        self.value_heads = nn.ModuleList([
            nn.Linear(hidden_size, 1) for _ in range(n_agents)
        ])
    ...
    def compute_advantages_and_returns(self, gamma: float, gae_lambda: float) -> None:
        ...
        for idxs in grouped.values():
            ...
            for i in reversed(idxs):
                ...
                delta = (
                    row["reward"]
                    + gamma_i * row["next_value"] * nonterminal
                    - row["value"]
                )
...
class SharedPPOTrainer:
    ...
    def _policy_obs_batch(self, obs_batch: np.ndarray, agent_ids: List[str]) -> np.ndarray:
        ...
        return np.stack(
            [append_agent_id_feature(obs, aid, self.agent_to_idx) for obs, aid in zip(obs_batch, agent_ids)],
            axis=0,
    ...
    def collect_rollout(
        ...
        for rollout_ep in range(n_rollout):
            ...
            while not da.done:
                ...
                actions_arr, logprobs_arr, values_arr, masks_arr, policy_obs_batch = self.act_batch_np(
                    obs_batch,
                    agent_ids,
                ...
                for i, aid in enumerate(agent_ids):
                    ...
                    self.buffer.add(
                        ...
                        agent_id=aid,
                        critic_obs=obs_at_action[aid],
                        agent_index=0,
                        episode_idx=rollout_ep,
                        step_idx=step_idx,
                        gamma=float(da.population.agents[aid].gamma),
                    )
...
class IndependentPPOTrainer:
    """
    Light-IPPO:
      - wspólny encoder obserwacji
      - niezależne głowy actor/critic per agent
    """
    ...
    def __init__(
        ...
        self.model = IndependentHeadPPOActorCritic(
            obs_dim=obs_dim,
            n_actions=n_actions,
        ...
        self.optimizer = torch.optim.Adam([
            {
                "params": list(self.model.trunk.parameters())
                       + list(self.model.policy_heads.parameters()),
                "lr": self.cfg.actor_lr,
            },
            {
                "params": self.model.value_heads.parameters(),
                "lr": self.cfg.critic_lr,
            },
        ])
        self.buffer = RolloutBuffer()
    ...
    def act_np(
        ...
        action, logprob, value = self.model.act(
            obs_t, mask_t, agent_idx=agent_idx, deterministic=deterministic
        )
        return int(action.item()), float(logprob.item()), float(value.item()), mask
    ...
    def act_batch_np(
        ...
        agent_indices = torch.as_tensor(
            [self.agent_to_idx[aid] for aid in agent_ids],
            dtype=torch.long,
            device=self.device,
        )
    ...
    def collect_rollout(
        ...
        for rollout_ep in range(n_rollout):
            ...
            while not da.done:
                obs_batch = da.get_observations(agent_ids)
                actions_arr, logprobs_arr, values_arr, masks_arr = self.act_batch_np(
                    obs_batch,
                    agent_ids,
                    deterministic=deterministic,
                )
                ...
                for i, aid in enumerate(agent_ids):
                    ...
                    self.buffer.add(
                        ...
                        action_mask=masks[aid],
                        agent_id=aid,
                        agent_index=self.agent_to_idx[aid],
                        episode_idx=rollout_ep,
                        step_idx=step_idx,
                        gamma=float(da.population.agents[aid].gamma),
                    )
    ...
    def load(self, path: str | Path) -> None:
        ...
        self.optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
        self.training_step = int(checkpoint.get("training_step", 0))
        self.agent_ids = checkpoint.get("agent_ids", self.agent_ids)
        self.agent_to_idx = checkpoint.get("agent_to_idx", self.agent_to_idx)
...
class MAPPOTrainer:
    def __init__(
        ...
        self.optimizer = torch.optim.Adam([
            {
                "params": list(self.model.actor_trunk.parameters()) + list(self.model.policy_head.parameters()),
                "lr": self.cfg.actor_lr,
            },
            {
                "params": list(self.model.critic_trunk.parameters()) + list(self.model.value_head.parameters()),
                "lr": self.cfg.critic_lr,
            },
        ])
        self.buffer = RolloutBuffer()
    ...
    def set_global_state(self, global_state: np.ndarray) -> None:
        self._last_global_state = np.asarray(global_state, dtype=np.float32)

    def _critic_obs(self, obs: np.ndarray, global_state: np.ndarray) -> np.ndarray:
        return np.concatenate([obs.astype(np.float32, copy=False), global_state.astype(np.float32, copy=False)]).astype(np.float32, copy=False)
    ...
    def act_np(
        ...
        if self._last_global_state is None:
            raise RuntimeError("MAPPOTrainer.act_np wymaga ustawionego global_state przed wyborem akcji")
        mask = action_mask_from_obs(obs)
        obs_t = torch.as_tensor(obs, dtype=torch.float32, device=self.device)
        mask_t = torch.as_tensor(mask, dtype=torch.bool, device=self.device)
        action, logprob = self.model.act(obs_t, mask_t, deterministic=deterministic)
        critic_obs = self._critic_obs(obs, self._last_global_state)
        critic_obs_t = torch.as_tensor(critic_obs, dtype=torch.float32, device=self.device).unsqueeze(0)
        value = self.model.critic_forward(critic_obs_t)
        return int(action.item()), float(logprob.item()), float(value.item()), mask
    ...
    def act_batch_np(
        ...
        obs_t = torch.as_tensor(obs_batch, dtype=torch.float32, device=self.device)
        mask_t = torch.as_tensor(masks, dtype=torch.bool, device=self.device)
        logits = self.model.actor_forward(obs_t)
        dist = masked_categorical(logits, mask_t)
        masked_logits = logits.masked_fill(~mask_t, MASK_VALUE)
        if deterministic:
            actions = torch.argmax(masked_logits, dim=-1)
            ...
            actions = dist.sample()
        logprobs = dist.log_prob(actions)
        global_repeat = np.repeat(global_state[None, :], len(obs_batch), axis=0).astype(np.float32, copy=False)
        critic_obs_batch = np.concatenate([obs_batch.astype(np.float32, copy=False), global_repeat], axis=1)
        critic_obs_t = torch.as_tensor(critic_obs_batch, dtype=torch.float32, device=self.device)
        values = self.model.critic_forward(critic_obs_t)
        return (
            actions.detach().cpu().numpy().astype(np.int32, copy=False),
            logprobs.detach().cpu().numpy().astype(np.float32, copy=False),
            values.detach().cpu().numpy().astype(np.float32, copy=False),
    ...
    @torch.no_grad()
    def value_np(self, obs: np.ndarray, global_state: np.ndarray) -> float:
        critic_obs = self._critic_obs(obs, global_state)
        critic_obs_t = torch.as_tensor(critic_obs, dtype=torch.float32, device=self.device).unsqueeze(0)
        value = self.model.critic_forward(critic_obs_t)
        return float(value.item())
    ...
    def collect_rollout(
        ...
        for rollout_ep in range(n_rollout):
            ...
            while not da.done:
                global_state = da.get_global_state()
                self._last_global_state = global_state
                obs_batch = da.get_observations(agent_ids)
                actions_arr, logprobs_arr, values_arr, masks_arr, critic_obs_batch = self.act_batch_np(
                    obs_batch,
                    global_state,
                    deterministic=deterministic,
                )
                obs_at_action = {aid: obs_batch[i] for i, aid in enumerate(agent_ids)}
                critic_obs_at_action = {aid: critic_obs_batch[i] for i, aid in enumerate(agent_ids)}
                actions_taken = {aid: int(actions_arr[i]) for i, aid in enumerate(agent_ids)}
                logprobs = {aid: float(logprobs_arr[i]) for i, aid in enumerate(agent_ids)}
                values = {aid: float(values_arr[i]) for i, aid in enumerate(agent_ids)}
                masks = {aid: masks_arr[i] for i, aid in enumerate(agent_ids)}
                ...
                for i, aid in enumerate(agent_ids):
                    if not masks_arr[i, actions_arr[i]]:
                        raise RuntimeError(f"MAPPO selected illegal action {int(actions_arr[i])} for {aid}")

                da.execute_parallel_actions(actions_taken)
                rewards, dones = da.compute_step_rewards()
                next_global_state = da.get_global_state()
                next_obs_batch = da.get_observations(agent_ids)
                next_values_batch = np.array(
                    [0.0 if bool(dones.get(aid, False)) else self.value_np(next_obs_batch[i], next_global_state)
                     for i, aid in enumerate(agent_ids)],
                    dtype=np.float32,
                )
                ...
                for i, aid in enumerate(agent_ids):
                    ...
                    self.buffer.add(
                        obs=obs_at_action[aid],
                        critic_obs=critic_obs_at_action[aid],
                        action=actions_taken[aid],
                        logprob=logprobs[aid],
                        value=values[aid],
                        reward=float(rewards.get(aid, 0.0)),
    ...
    def update(self) -> Dict[str, float]:
        ...
        for _ in range(self.cfg.update_epochs):
            for batch in self.buffer.get_minibatches(self.cfg.minibatch_size, self.device):
                ...
                logits = self.model.actor_forward(batch["obs"])
                values = self.model.critic_forward(batch["critic_obs"])
                dist = masked_categorical(logits, batch["action_masks"])
                new_logprobs = dist.log_prob(batch["actions"])
                entropy = dist.entropy().mean()
